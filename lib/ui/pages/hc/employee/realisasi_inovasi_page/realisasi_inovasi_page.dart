...
import 'package:bumn_eid/ui/pages/hc/employee/realisasi_inovasi_page/di/realisasi_inovasi_page_graph.dart';
...
class RealisasiInovasiData with ListBarChartDataType {
  final int tahun;
  final String jenis; //rencana & realisasi
  final double jumlah;

  RealisasiInovasiData({
    @required this.tahun,
    @required this.jenis,
...
class RealisasiInovasiPage extends StatefulWidget with InjectableWidget {
  final RealisasiInovasiPageGraph _graph;
  ...
  //injected
  RealisasiInovasiActionMapper actionMapper;
  //injected
  HcController hcController;

  @override
  RealisasiInovasiPageGraph graph() => _graph;

  RealisasiInovasiPage(
      {@required this.company, RealisasiInovasiPageGraph graph})
      : this._graph = graph ?? RealisasiInovasiPageGraph() {
    setup();
  }

  @override
  _RealisasiInovasiPageState createState() => _RealisasiInovasiPageState();
}

class _RealisasiInovasiPageState extends State<RealisasiInovasiPage>
    with AfterLayoutMixin<RealisasiInovasiPage> {
  Map<int, List<RealisasiInovasiData>> chartData;
  ...
  void getData() async {
    try {
      ...
      final data = await widget.hcController
          .fetchAnggaranInovasiByCompany(companyId: widget.company.getId());
