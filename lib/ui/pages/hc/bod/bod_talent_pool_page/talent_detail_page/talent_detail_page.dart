...
import 'package:bumn_eid/ui/pages/hc/bod/bod_talent_pool_page/talent_detail_page/di/talent_detail_page_graph.dart';
...
class TalentDetailPage extends StatefulWidget with InjectableWidget {
  final TalentDetailPageGraph _graph;
  ...
  //injected
  TalentDetailActionMapper actionMapper;

  TalentDetailPage({@required this.id, TalentDetailPageGraph graph})
      : this._graph = graph ?? TalentDetailPageGraph() {
    setup();
  }

  @override
  TalentDetailPageGraph graph() => _graph;

  @override
  _TalentDetailPageState createState() => _TalentDetailPageState();
}

class _TalentDetailPageState extends State<TalentDetailPage>
    with AfterLayoutMixin<TalentDetailPage> {
  bool isError = false;
  ...
  void getData() async {
    try {
      ...
      final fetchedTalent =
          await widget.hcController.fetchTalentDetail(id: widget.id);

      setState(() {
        this.talent = fetchedTalent;
      });
    } catch (ex) {
      print(ex.toString());
      setState(() {
