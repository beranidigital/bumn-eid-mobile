...
import 'package:bumn_eid/ui/pages/hc/dekom/potret_tugas/potret_tugas_detailed_page/di/potret_tugas_detailed_page_graph.dart';
...
class PotretTugasDetailedPage extends StatefulWidget with InjectableWidget {
  final PotretTugasDetailedPageGraph _graph;
  ...
  //injected
  PotretTugasDetailedActionMapper actionMapper;
  //injected
  HcController hcController;

  @override
  PotretTugasDetailedPageGraph graph() => _graph;

  PotretTugasDetailedPage(
      {@required this.range,
      @required this.now,
      PotretTugasDetailedPageGraph graph})
      : this._graph = graph ?? PotretTugasDetailedPageGraph() {
    setup();
  }

  @override
  _PotretTugasDetailedPageState createState() =>
      _PotretTugasDetailedPageState();
}

class _PotretTugasDetailedPageState extends State<PotretTugasDetailedPage>
    with AfterLayoutMixin<PotretTugasDetailedPage> {
  ...
  void getData() async {
    try {
      ...
      final data = await widget.hcController
          .fetchGrafikPotretTugasDetailed(range: widget.range, now: widget.now);
  ...
  Widget listItem(GrafikPotretTugasDetailed data) {
    return GestureDetector(
      onTap: () =>
          widget.actionMapper.openTalentPage(id: data.id, now: widget.now),
      child: Container(
        height: 100.0,
        child: Card(
