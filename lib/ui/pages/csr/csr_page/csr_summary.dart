...
class CsrSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LoadableWidget(
      fetchFunction: () async {
        return csrController.fetchCsrDetailAll(
          tahun: '2020',
        );
      },
      colorPalette: ColorPalette(),
      buildFunction: (ctx, generalData) {
        final List<CsrItem> items = generalData;
        return InkWell(
          onTap: () {
            ApiStatistic().insertStatistic("CSR", "Level 2 List CSR Summary");
            StoreProvider.of<AppState>(context).dispatch(
              NavigateToNextAction(
                destination: CsrCompanyListPageDestination(),
              ),
            );
          },
          child: BaseCard(
