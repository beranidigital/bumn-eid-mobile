...
import 'package:bumn_eid/ui/pages/csr/bina_lingkungan/bl_company_detailed/di/bl_company_detailed_page_graph.dart';
...
class BLCompanyDetailedPage extends StatefulWidget with InjectableWidget {
  final BLCompanyDetailedPageGraph _graph;
  ...
  //injected
  BLCompanyDetailedActionMapper actionMapper;
  //injected
  ...
  @override
  BLCompanyDetailedPageGraph graph() => _graph;

  BLCompanyDetailedPage(
      {@required this.companyId, BLCompanyDetailedPageGraph graph})
      : this._graph = graph ?? BLCompanyDetailedPageGraph() {
    setup();
  }

  @override
  _BLCompanyDetailedPageState createState() => _BLCompanyDetailedPageState();
}

class _BLCompanyDetailedPageState extends State<BLCompanyDetailedPage>
    with AfterLayoutMixin<BLCompanyDetailedPage> {
  ...
  void getData() async {
    try {
      ...
      List<PkItemCompany> companiesList = await widget.csrController
          .fetchBlItemByCompany(companyId: widget.companyId);
  ...
  Widget build(BuildContext context) {
    String title = "Penyebaran Wilayah BL";
    return BaseScaffold(
      title: title,
